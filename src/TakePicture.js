...
export default class TakePicture extends React.Component {
  ...
  async fetchWordData(queries, db, words){ 
     var newWords = words;
     await db.transaction(tx => {
       ...
       Promise.all(queries.map(async (q) => {
          try {
              switch(q.type){
                  case 'SELECT':                      
                 await tx.executeSql(q.sql, q.vals,function(tx, res) {
                    
                  //console.log('Query', q, 'Executed. results:', results);
                  var word = res.rows.item(0);
                  if (typeof(word) !== 'undefined' || word != null) {
                    //console.log(word.viet);
                    newWords[word.viet] = word.anh;
                  }
                  //const { rows } = results;
    ...
    if (this.camera) {
      ...
      for(var key in deviceTextRecognition){  
       /* while ((m = regex.exec(deviceTextRecognition[key].blockText)) !== null) {
        ...
        let query = {
          type: 'SELECT',
          sql: 'SELECT viet, anh FROM words where viet LIKE (?)',
          vals: [t],
        }
        queries.push(query);
      ...
      var db = await SQLite.openDatabase({name : "anhviet.db", createFromLocation : 1}, this.openCB, this.errorCB );   
      var words = await this.fetchWordData(queries, db, words);
      db.close();
      

       for(var key in words){  
          lessons.push([key, words[key] , '' ]);
       }
       this.props.navigation.navigate('ShowLesson',{ lessons },);
